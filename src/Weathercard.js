...
export default function Weathercard() {
  ...
  useEffect(() => {
    ...
    const getWeather = async () => {
      try {
        const api_call = await fetch(`${API_URL}/weather?q=Melbourne,AU&APPID=${API_key}`);
        ...
        const daily_api = await fetch(`${API_URL}/onecall?lat=${lat}&lon=${long}&appid=${API_key}`);
  ...
  return (
    <>
      <Grid container className={classes.root} spacing={2}>
        <Grid className={classes.introduction} item xs={12} sm={7} md={7} lg={4} xl={4}>
            
          <Box className={classes.currentCity}>
            <Typography variant='h6' className={classes.text}> CURRENTLY </Typography>
            <Typography variant='h6' className={classes.cityText}>{data.name} </Typography>
          </Box>
          <Box> 
            <Typography variant='h6' className={classes.description}>{description.charAt(0).toUpperCase() + description.slice(1)}</Typography>
          </Box>
        ...
        <Grid className={classes.introductions} item xs={12} sm={5} md={4} lg={2} xl={2}>
          <Typography variant='h2' className={classes.number}> {data.main ? calCelsius(data.main.temp): null}<span className={classes.span}>&deg;C</span> </Typography>
        </Grid>
        <Grid item xs={12} sm={5} md={4} lg={11} xl={2} >
          <hr className={classes.dottedLine}></hr>
        </Grid>
        <Hourlyforecast data={DailyData}  ids={dailyIconId} calCelsius={calCelsius} />
      </Grid>
